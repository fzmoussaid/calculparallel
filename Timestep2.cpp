...
{
	int decal(me > 0), lastLine(nyLocal + decal + (me < np-1) - 1);
	Rhs.setZero();
	//---------------- Contribution du bord haut -----------------------------------------------------------------------
	if(me < np-1) {
		for(int i(0); i < nx; i++)
		{
			Rhs(bijection(i,lastLine,nx)) = -gamma*Uhaut(i);
		}
	} else {
		for(int i(0); i < nx; i++)
		{
			Rhs(bijection(i,lastLine,nx)) = -gamma*functionG((i+1)*dx,ly,t,p);
		}
	}
	...
	for (int j = decal; j < nyLocal+decal; j++)
	{
		...
		{
			Rhs(bijection(i,j,nx)) += functionF((i+1)*dx,yi,t,p);
		}
		Rhs(bijection(0,j,nx)) -=  beta*functionH(0.,yi,t,p);
		Rhs(bijection(nx-1,j,nx)) -= beta*functionH(lx,yi,t,p);
	}

	//---------------- Schéma d'Euler explicite -------------------------------------------------------------------------
	Rhs.segment(nx*nyLocal,nx*decal) += Un.segment(nx*nyLocal,nx*decal)/dt;
}
...
int timeStep(const SolverCG& var, VectorXd& Un, double eps, double beta, double gamma, double t, int Niter, int recouvr, int me, int np, int i1, int im)
{
	...
	int nyLocal(im-i1+1);
	VectorXd Ubas(nx), Uhaut(nx), UbasNext(nx), UhautNext(nx), Unext(Un.size()), Rhs(Un.size());
